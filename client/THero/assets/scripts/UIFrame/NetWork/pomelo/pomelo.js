...
(function () {
  ...
  Emitter.prototype.emit = function (event) {
    ...
    if (callbacks) {
      ...
      cc.warn("@@@not event callBacks:", event);
    }
...
(function (exports, ByteArray, global) {
  ...
  Message.decode = function (buffer) {
    ...
    // console.log("msgDecode_bodyLen:",bodyLen,",byteLen:",bytesLen);
...
(function () {
  ...
  var gapThreshold = 200; // heartbeat gap threashold
  ...
  var isKicked = false;
  var onKick = function onKick(event) {
    isKicked = true;
  };
  ...
  var connect = function connect(params, url, cb) {
    ...
    var onclose = function onclose(event) {
      ...
      intervalHandle = setInterval(function () {
        if (socket && socket.readyState != WebSocket.CLOSED) {
          console.log('socket.readyState1: ', socket.readyState);
          if (socket.close) {
            socket.close();
          }
          console.log('socket.readyState2: ', socket.readyState);
        } else {
          console.log('true close socket ');
          if (socket) {
            socket.onopen = null;
    ...
    if (!!params.reconnect) {
      //重连的次数
      pomelo.reconnect = function () {
        console.log("pomelo reconnect isKicked="+isKicked);
        if (!isKicked) {
          //发起重连
          if (reconnectAttempts < maxReconnectAttempts) {
            ...
            reconncetTimer = setTimeout(function () {
              console.log("reconnnect times:", reconnectAttempts);
              pomelo.emit('reconnect', reconnectAttempts);
  ...
  pomelo.disconnect = function (cb) {
    ...
    if (socket) {
      ...
      console.log('disconnect socket.readyState3: ', socket.readyState);
      socket.onopen = null;
      socket.onmessage = null;
      socket.onerror = null;
      socket.onclose = null;
  ...
  var deCompose = function deCompose(msg) {
    ...
    if (msg.compressRoute) {
      // console.log("msg.compressRoute:",route)
      if (!abbrs[route]) {
        return {};
      }
    ...
    if (protobuf && serverProtos[route]) {
      // console.log("protobuf:",route)
      return protobuf.decode(route, msg.body);
    } else if (decodeIO_decoder && decodeIO_decoder.lookup(route)) {
      // console.log("decodeIO_decoder:",route)
      return decodeIO_decoder.build(route).decode(msg.body);
    } else {
      var msgDataStr = Protocol.strdecode(msg.body);
      //最可以信任的网络协议日志
      // console.log("trust-net-back:",route,"body:",msgDataStr);
      return JSON.parse(msgDataStr);
    }
