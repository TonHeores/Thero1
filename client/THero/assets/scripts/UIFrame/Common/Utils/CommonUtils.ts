...
import { Node, Sprite,Event, SpriteFrame, Vec2,loader,Label } from "cc";
...
export class CommonUtils {
    ...
    public static setItemSpriteFrame(sprite:Sprite, url:string, successCB:(sprite:Sprite)=>void = null) {
        sprite["spriteFrameName"] = url;
        CocosHelper.loadResSync(url, SpriteFrame).then((spriteFrame:SpriteFrame)=>{
    ...
    //     targetNode.on(Node.EventType.TOUCH_START, (e:Event.EventTouch)=>{
    //         lastTouchPos = e.getLocation();
    //     }, this);
    //     targetNode.on(Node.EventType.TOUCH_END, (e:Event.EventTouch)=>{
    //         if(lastTouchPos) {
